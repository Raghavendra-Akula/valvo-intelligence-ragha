...
valvo_ai_v7_bp = Blueprint("valvo_ai_v7", __name__)
...
@valvo_ai_v7_bp.route("/api/valvo-ai-v7/query", methods=["POST"])
...
@valvo_ai_v7_bp.route("/api/valvo-ai-v7/stream", methods=["POST"])
@limiter.limit("20 per minute")
def stream():
...
@valvo_ai_v7_bp.route("/api/valvo-ai-v7/history-list", methods=["GET"])
@limiter.limit("60 per minute")
def history_list():
    """Return conversations grouped by session gaps (>30 min = new conversation)."""
    from database.database import get_db
    from services.valvo_ai_v7.engine import _history_scope

    page_context = (request.args.get("page_context") or "valvo-ai-v7").strip() or "valvo-ai-v7"
...
@valvo_ai_v7_bp.route("/api/valvo-ai-v7/health", methods=["GET"])
@limiter.limit("60 per minute")
def health():
    from services.valvo_ai_v7.engine import ValvoAIV7Engine
...
@valvo_ai_v7_bp.route("/api/valvo-ai-v7/history", methods=["DELETE"])
