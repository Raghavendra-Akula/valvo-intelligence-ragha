...
valvo_ai_v3_bp = Blueprint("valvo_ai_v3", __name__)
...
@valvo_ai_v3_bp.route("/api/valvo-ai-v3/query", methods=["POST"])
...
def query():
    ...
    from services.valvo_ai_v3.engine import ValvoAIV3Engine

    engine = ValvoAIV3Engine()
    result = engine.query(
        message=message,
...
@valvo_ai_v3_bp.route("/api/valvo-ai-v3/stream", methods=["POST"])
@limiter.limit("20 per minute")
def stream():
...
@valvo_ai_v3_bp.route("/api/valvo-ai-v3/history-list", methods=["GET"])
@limiter.limit("60 per minute")
def history_list():
    """Return conversations grouped by session gaps (>30 min = new conversation)."""
    from database.database import get_db
    from services.valvo_ai_v3.engine import _history_scope

    page_context = (request.args.get("page_context") or "valvo-ai-v3").strip() or "valvo-ai-v3"
...
@valvo_ai_v3_bp.route("/api/valvo-ai-v3/health", methods=["GET"])
@limiter.limit("60 per minute")
def health():
    from services.valvo_ai_v3.engine import ValvoAIV3Engine
...
@valvo_ai_v3_bp.route("/api/valvo-ai-v3/history", methods=["DELETE"])
