"""
Valvo AI v5 -- Embedded database schema (cloned from v3).
"""
...
stock_fundamentals: security_id(text), symbol(text), company_name(text), sector(text), industry(text), market_cap_cr(float), shares_outstanding(bigint), updated_at(timestamp)
...
# INDEX DAILY SUMMARY — per-index snapshot, refreshed from candles_indices
index_daily_summary: symbol(text PK), category(text) -- 'sector'|'broad'|'thematic'|'other', prev_close(float), ma50(float), ma200(float), high_52w(float), low_52w(float), ath(float), close_5d(float), close_20d(float), close_60d(float), close_120d(float), close_252d(float), return_5d(float), return_20d(float), return_60d(float), return_252d(float), above_ma50(bool), above_ma200(bool), leadership_score(float) -- sector only, computed_date(date), updated_at(timestamp)

# MARKET BREADTH / MULTI-TIMEFRAME CANDLES
...
breadth_intraday: id(bigint), date(date), time_ist(time), advances(int), declines(int), total(int), sampled_at(timestamp)
candles_weekly: security_id(text), date(date), open(numeric), high(numeric), low(numeric), close(numeric), volume(bigint)
candles_monthly: security_id(text), date(date), open(numeric), high(numeric), low(numeric), close(numeric), volume(bigint)

# PRICE ALERTS
price_alerts: id(int), user_id(text), security_id(text), symbol(text), condition(text) -- 'above'|'below', threshold(float), last_price(float), active(bool), triggered(bool), triggered_at(timestamp), last_notified_at(timestamp), trigger_count(int), recurring(bool), cooldown_hours(int), notification_method(text), notes(text), created_at(timestamp)
"""
...
MARKET REGIME:
  The CURRENT market regime is the latest row in market_regime_history (append-only log).
    SELECT regime, note, updated_at FROM market_regime_history ORDER BY updated_at DESC LIMIT 1
  Regime values are short labels (e.g. 'bull', 'sideways', 'grind_down', 'sharp_downtrend').
  Each live position ALSO stores its own market_regime snapshot in the positions table.
  When the user asks "how do my positions stand in the current regime":
    1. Fetch current regime from market_regime_history.
    2. Use get_positions to list active positions with their per-position regime tags.
    3. Compare and comment on which positions align with or diverge from the current regime.

LEADING SECTORS:
  Primary source: index_daily_summary (refreshed from candles_indices) — each sector
  index (NIFTY METAL, NIFTY PHARMA, NIFTY IT, …) gets a row with 5d/20d/60d/252d
  return, MA50/MA200 alignment, and a composite leadership_score. get_leading_sectors
  tool returns the top N by leadership_score automatically.
  For "sector X return over last N days" questions use sql_query on index_daily_summary
  (single joined row) or candles_indices (multi-day series).

  Legacy: leading_sectors table (append-only, used to be manually set in Settings).
  Treat as fallback only — normal flow should use the tool.

  When the user asks "my positions in leading sectors" / "which of my stocks
  are in hot sectors" / "is X a metal stock" → USE THE
  get_positions_sector_membership TOOL. It joins positions ↔ index_constituents
  by security_id internally (the only reliable key — see CROSS-REF block below)
  and returns each position's sector-index memberships plus a flag for the
  currently leading ones. One tool call, no risk of missed matches.

CROSS-REF: POSITIONS ↔ SECTOR INDEX (if you ever must write SQL yourself):
  positions.stock_name stores human display names ("HITACHI ENERGY INDIA LTD").
  index_constituents.stock_symbol stores NSE tickers ("POWERINDIA").
  stock_fundamentals currently has no rows — do NOT fall back to it.
  ALWAYS join by security_id (shared integer key):
    SELECT p.stock_name, ic.index_symbol, ic.sector
    FROM positions p
    JOIN index_constituents ic ON ic.security_id = p.security_id
    WHERE p.status='active' AND p.user_id = <uid>
      AND ic.index_symbol IN ('NIFTY METAL','NIFTY ENERGY',...);
  Example misses from wrong joins: NALCO ↔ NATIONALUM, HITACHI ENERGY INDIA LTD
  ↔ POWERINDIA, RELIANCE INDUSTRIES LIMITED ↔ RELIANCE.

SECTOR MATCHING (submissions.sector, journal_trades.sector, index_constituents.sector):
  Sector names are stored with mixed capitalisation ("Metal", "Metals", "METAL", "Information Technology", etc.).
  ALWAYS use ILIKE '%keyword%' for sector filters, never '=' on a user-typed sector name.
    WHERE sector ILIKE '%metal%'
  If a sector lookup returns 0 rows, retry with shorter/looser keywords before giving up.

SCORED STOCKS (submissions table):
  "scored stocks in X sector" → SELECT stock_name, final_score, rating, sector, timestamp
    FROM submissions WHERE sector ILIKE '%X%' ORDER BY final_score DESC LIMIT 20
  If empty: broaden the keyword, or fall back to latest submissions regardless of sector and mention the miss.
...
FUNDAMENTALS:
  ...
  Use get_fundamentals tool (not raw sql) for "what's the P/E of RELIANCE", "show me
  TCS fundamentals", "compare ROCE of INFY and WIPRO" — it accepts a symbol and
  returns the full overview row plus peer list. Join key: symbol OR security_id.

  For time-series line items use financials_quarterly (last ~20 quarters) or
  financials_annual (10+ years of P&L + balance sheet + cash flow). period_end_date
  is the as-of date; filing_date is when the company reported. is_consolidated=true
  rows are preferred when available. ALWAYS SELECT period_end_date — the `period`
  text column is unreliable (often literally "Quarterly" for newly-listed names like
  GROWW). Example: "last 4 quarter revenue of X" →
    SELECT period_end_date, period, revenue_cr, net_profit_cr, opm_percent
    FROM financials_quarterly
    WHERE symbol ILIKE '%X%' AND is_consolidated = true
    ORDER BY period_end_date DESC LIMIT 4

  Peers: SELECT peer_symbol FROM peers WHERE symbol = 'X' ORDER BY relevance_rank
  Corporate actions: SELECT action_type, ex_date, dividend_amount, bonus_ratio,
    split_ratio FROM corporate_actions WHERE symbol = 'X' ORDER BY ex_date DESC
  Shareholding: latest row from shareholding_quarterly, ordered by period_end_date DESC.
  Segments: financials_quarterly tells you company total; segments_quarterly breaks it
    down by business segment for the same period_end_date.
  Filings: filings table has links to the raw PDF/XBRL for audit trails.

MARKET BREADTH:
  breadth_daily_history is the EoD snapshot (one row per trading day) with advance/
  decline, % of universe above EMA20/50/200, new highs/lows, and a JSONB
  sector_breadth map (key=sector name, value=breadth stats).
  ...
  breadth_intraday is the live A/D tick ticker (many rows per day), use only when
  the user asks about today's live breadth. For historical breadth always use
  breadth_daily_history.

WEEKLY / MONTHLY CANDLES:
  candles_weekly / candles_monthly mirror candles_daily's shape but aggregated.
  Use these when the user asks for multi-year context ("weekly chart of X",
  "X monthly high over 10 years"). Same security_id join as candles_daily.

PRICE ALERTS (per-user via RLS):
  price_alerts is the user's own alert list. "my alerts" / "what alerts have I set"
  → SELECT symbol, condition, threshold, active, triggered, triggered_at,
    last_price FROM price_alerts ORDER BY created_at DESC
  Filter active=true for currently-armed alerts, triggered=true for fired ones.
...
SCREENER / SCANNER DATA:
  ...
  - liq_cr = average daily liquidity in Crores (19-day average turnover / 1e7)
