"""
Valvo AI v7 -- Tool definitions and executors.
...
GET_FLAGGED_ITEMS_TOOL = {
    "name": "get_flagged_items",
    "description": (
        "Fetch all stocks the user has flagged across their watchlists. Flags "
        "are the user's own priority/execution markers — when they ask 'what "
        "are my flagged stocks', 'my priority stocks', 'stocks I starred for "
        "execution', 'shortlist', or want a quick scan of their high-priority "
        "names, call this. Returns each row with symbol, company name, the "
        "watchlist it lives in, and the user's notes — use the notes for "
        "context (often contains the reason it was flagged). Best paired with "
        "get_live_market when the user wants current prices on the shortlist."
    ),
    "input_schema": {
        "type": "object",
        "properties": {},
...
def _exec_get_flagged_items(params: dict) -> dict:
    uid = _get_user_id()
    if not uid:
        return {"type": "flagged_items", "count": 0, "items": [],
                "warning": "No authenticated user — cannot load flagged items"}
    conn = get_db()
    if not conn:
        return {"error": "Database unavailable"}
    try:
        ...
        cur.execute(
            """
            SELECT wi.symbol, wi.company_name, wi.security_id, wi.notes,
                   wi.section_name, wi.added_at,
                   w.id   AS watchlist_id,
                   w.name AS watchlist_name
            FROM watchlist_items wi
            JOIN watchlists w ON w.id = wi.watchlist_id
            WHERE wi.flagged = TRUE AND w.user_id = %s
        ...
        return {"error": f"Flagged items error: {exc}"}
    finally:
        close_db(conn)
...
def _exec_v2_action(action_name: str, params: dict) -> dict:
    ...
    return to_jsonable(run_action(action_name, params, request_text="valvo-ai-v7"))
...
_EXECUTOR_MAP: dict[str, Any] = {
    ...
    "get_flagged_items": _exec_get_flagged_items,
